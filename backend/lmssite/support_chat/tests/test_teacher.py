...
from support_chat.models import UnauthorizedTickets, StudentTickets, TeacherTickets
...
class LecturesTestsGuest(APITestCase):
    def setUp(self):
        ...
        url = reverse('teacher_ticket-create')
        response = self.client.post(url, self.tickets_props, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(TeacherTickets.objects.count(), 1)
        self.assertEqual(TeacherTickets.objects.get().title, 'newTitle')
    ...
    def test_lectures_create(self):
        self.client.force_authenticate(user=self.teacher_user)
        # GET All
        url = reverse('teacher_ticket-list')
        ...
        # GET ONE
        url = reverse('teacher_ticket-id', kwargs={'pk': 1})
        response = self.client.get(url)
        ...
        # UPDATE
        url = reverse('teacher_ticket-rud', kwargs={'pk': 1})
        update_data = {'title': 'updated'}
        response = self.client.put(url, update_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(TeacherTickets.objects.count(), 1)
        self.assertEqual(TeacherTickets.objects.get().title, 'updated')

        # DELETE
        url = reverse('teacher_ticket-rud', kwargs={'pk': 1})
        response = self.client.delete(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(TeacherTickets.objects.count(), 0)