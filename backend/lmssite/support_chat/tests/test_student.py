...
from support_chat.models import UnauthorizedTickets, StudentTickets
...
class LecturesTestsGuest(APITestCase):
    def setUp(self):
        ...
        url = reverse('student_ticket-create')
        response = self.client.post(url, self.tickets_props, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(StudentTickets.objects.count(), 1)
        self.assertEqual(StudentTickets.objects.get().title, 'newTitle')
    ...
    def test_lectures_create(self):
        self.client.force_authenticate(user=self.student_user)
        # GET All
        url = reverse('student_ticket-list')
        ...
        # GET ONE
        url = reverse('student_ticket-id', kwargs={'pk': 1})
        response = self.client.get(url)
        ...
        # UPDATE
        url = reverse('student_ticket-rud', kwargs={'pk': 1})
        update_data = {'title': 'updated'}
        response = self.client.put(url, update_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(StudentTickets.objects.count(), 1)
        self.assertEqual(StudentTickets.objects.get().title, 'updated')

        # DELETE
        url = reverse('student_ticket-rud', kwargs={'pk': 1})
        response = self.client.delete(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(StudentTickets.objects.count(), 0)
